...
/**
 * 版本1.3， 当小倍位里有2个以上的箱子要做单箱吊的时候，则该舱重新排序列
 * Created by csw on 2016/3/8.
 */
class GeneratePreStowageFromKnowStowage3 {
...
//
                ...
                if(VBY_BAYIDs.size() == 2) {//舱内有两个倍，一般全是20尺的箱子
                    ...
                    List<PreStowageData> resultReturn = this.workBayNumIsThree(seq, dataList, smallBay, largeBay, VTR_TIERNOs, VRW_ROWNO_D_20, stringListMap_D_20, VRW_ROWNO_L_20,  stringListMap_L_20,
                            VRW_ROWNO_D_40, stringListMap_D_40, VRW_ROWNO_L_40, stringListMap_L_40, resultList)
                    //统计结果中20尺、作业工艺为1的箱子的个数
                    int num = 0
                    for(PreStowageData preStowageData1 : resultReturn) {
            ...
            for(Integer tier : tiers) {
                ...
                if(tier < 82) {
                    ...
                    for(Integer row : VRW_ROWNO_D_40) {
                        stringListMap_D_40.get(largeBay.get(0)+"."+tier+"."+row).setMOVE_ORDER(seq)
                        stringListMap_D_40.get(largeBay.get(0)+"."+tier+"."+row).setWORKFLOW(1+"")
                        seq++
                    }
...
//                                        println "倍.层.排"+VBY_BAYIDs.get(1)+"."+tier+"."+odd1.get(p)
    ...
    public static List<PreStowageData> workBayNumIsThree(int seq, List<PreStowageData> dataList, List<Integer> smallBay, List<Integer> largeBay, List<Integer> VTR_TIERNOs,
                                      List<Integer> VRW_ROWNO_D_20, Map<String, PreStowageData> stringListMap_D_20,
                                      List<Integer> VRW_ROWNO_L_20, Map<String, PreStowageData> stringListMap_L_20,
                                      List<Integer> VRW_ROWNO_D_40, Map<String, PreStowageData> stringListMap_D_40,
                                      List<Integer> VRW_ROWNO_L_40, Map<String, PreStowageData> stringListMap_L_40,
